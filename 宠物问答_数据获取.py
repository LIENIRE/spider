# -*- coding: utf-8 -*-
"""
Created on Tue Aug  2 16:06:56 2022
...
#url = 'http://www.boqii.com/baike/feeding/'
#URL='http://www.boqii.com/baike/dog/'
...
originurl=tcurl[0]
starturl='http://www.boqii.com/baike/catfd-330/'
...
print(type(driver))
#首页
firstpage = re.findall(r'\d+',starturl)
if firstpage==[]:
    firstpage=1
else:
...
for page in range(firstpage,finalpage+1):
    ...
    awzurl=driver.find_element(By.CLASS_NAME,'art_list').find_elements(By.TAG_NAME,'dt')#每页每条数据的网址
    ...
    for num in range(1,len(urls)+1):
        ...
        ejbq = driver.find_element(By.ID,'content').find_element(By.CLASS_NAME,'content_auto').find_elements(By.TAG_NAME,'a')[3].text
        adbq.append(dbq)
        ayjbq.append(yjbq)
        aejbq.append(ejbq)
        ...
        zwnr=driver.find_element(By.CLASS_NAME,'article_body').text
        abt.append(bt)
        azy.append(zy)
        awzny.append(zwnr)
...
workbook = xlsxwriter.Workbook(dbq+'-'+yjbq+'1.xlsx')
...
head=['大分区','一级分区','二级分区','链接','标题','摘要','正文内容']
...
worksheet.write_column(1, 6, alllist[6])
